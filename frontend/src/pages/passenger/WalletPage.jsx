// ============================================================
// PROJO GROUP — Wallet Page (FIXED)
// FIX 1: walletAPI.getBalance() returns { wallet } not wallet directly
// FIX 2: PayFast top-up fully wired — redirects to PayFast
// ============================================================
...
export default function WalletPage() {
  ...
  async function loadWallet() {
    setLoading(true);
    try {
      // FIX: API returns { wallet: {...} } — extract .wallet
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.6rem",
            fontWeight: "800", color: "#f0ede8", margin: 0 }}>PROJO Wallet</h1>
        </div>
        ...
        <div style={{
          ...
          {loading ? (
            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: "3rem",
              fontWeight: "800", color: "#3d3d3d" }}>R—</div>
          ) : (
          ...
          <div style={{ display: "flex", gap: "1.5rem", marginTop: "1.25rem", flexWrap: "wrap" }}>
            <div>
              <div style={{ fontSize: "11px", color: "#6b6760", fontWeight: "600" }}>
                Loyalty Points
              </div>
              <div style={{ fontSize: "18px", fontWeight: "700", color: G }}>
                ⭐ {points}
              </div>
            </div>
            <div>
        ...
        {/* Top up */}
        <div style={{ background: BG2, border: `1px solid rgba(232,184,75,0.15)`,
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px", flexWrap: "wrap" }}>
            {QUICK_AMOUNTS.map(amt => (
              <button key={amt} onClick={() => setTopupAmt(String(amt))} style={{
                ...
                background: topupAmt === String(amt) ? G : BG3,
                color: topupAmt === String(amt) ? "#0a0a0a" : "#a8a49e",
                border: topupAmt === String(amt) ? "none" : `1px solid ${BORDER}`,
              }}>R{amt}</button>
            ))}
          </div>

          <div style={{ display: "flex", gap: "10px" }}>
            <div style={{ display: "flex", flex: 1, background: BG3,
              ...
              <input value={topupAmt} onChange={e => setTopupAmt(e.target.value)}
                placeholder="Enter amount" type="number" min="10"
            ...
            <button onClick={handleTopup} disabled={topping} style={{
              background: topping ? "#9a7520" : G, color: "#0a0a0a", border: "none",
        ...
        <div style={{ background: BG2, border: `1px solid rgba(232,184,75,0.15)`,
          ...
          <div>
            ...
            <div style={{ fontSize: "12px", color: "#6b6760" }}>
              Share PROJO GROUP. Both you and your friend get R50 wallet credit.
            </div>
          </div>
          <button onClick={() => {
            const msg = encodeURIComponent(
              `Join PROJO GROUP — Rustenburg's ride, delivery & services app! Get R50 wallet credit when you sign up. ${CONTACT.website}`
            );
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {txs.map(tx => (
              <div key={tx.id} style={{
                background: BG2, border: "1px solid rgba(232,184,75,0.1)",
                ...
                <div style={{
                  ...
                  color: CREDIT_TYPES.includes(tx.type) ? "#4ade80" : G,
                }}>
