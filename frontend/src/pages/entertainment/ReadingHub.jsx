...
function BookReader({ book, onClose }) {
  ...
  useEffect(() => {
    ...
    const textUrl = `https://www.gutenberg.org/cache/epub/${book.gutenbergId}/pg${book.gutenbergId}.txt`;
    fetch(`https://projo-group-backend.onrender.com/api/news/proxy?url=${encodeURIComponent(textUrl)}`)
      .then(r => r.text())
      .then(data => {
        // Clean up Gutenberg header/footer
        const start = data.indexOf("*** START OF");
        const end = data.indexOf("*** END OF");
        const cleaned = start > -1 ? data.slice(start + 50, end > -1 ? end : undefined) : data;
        setText(cleaned.slice(0, 50000)); // first 50k chars
        setLoading(false);
      })
      .catch(() => {
        ...
        fetch(textUrl)
          ...
          .catch(() => { setText("Could not load book. Please try again."); setLoading(false); });
      });
